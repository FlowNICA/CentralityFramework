...
void printFinal(TString inFileName = "file:///home/dim/Work/FIT/dataNEW/FITGlaub/gen_AuAu9/gamma.root", TString outFileName = "")
{
    if (inFileName == "")
        return;
    ...
    if (isTypeSimple)
    {
        ...
        std::cout << std::endl;
        std::cout << "const float BinN[Nb+1]={";
        for (int i = 0; i < NreasonableClasses; i++)
        {
            if(i==NreasonableClasses-1)
            std::cout << vBorders.at(i).first << "";
            else
            std::cout << vBorders.at(i).first << ",";
        }
        std::cout << "};"<< std::endl;

        std::cout << "const float Binb[Nb+1]={";
        for (int i = 0; i < NreasonableClasses; i++)
        {
            if(i==NreasonableClasses-1)
            std::cout << Form("%4.2f",vBimp.at(i).second )<< "";
            else
            std::cout << Form("%4.2f",vBimp.at(i).second ) << ",";
        }
        std::cout << "};"<< std::endl;;
