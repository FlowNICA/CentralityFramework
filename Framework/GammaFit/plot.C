...
void plot() //Результаты фитирования множественности с ратио и классами центральности для Глаубера
{
  ...
  const char *outDir = "/home/dim/Work/QA/OUT_Gl/Set1/";
  ...
  TFile *fileIn = new TFile("file:///home/dim/Work/QA/BiBi_UrQMDreq25.root"); // file with modal data mult vs b

  TH1F *HistData = (TH1F *)fileFitGl->Get("hRefMultSTAR_set1");
  ...
  TH2F *hBvsRefMultFit = (TH2F *)fileFitGl->Get("B_VS_Multiplicity"); // B_VS_Multiplicity");
  TH2F *hBvsRefMult = (TH2F *)fileIn->Get("hBvsRefMult_set1");

  TH1F *Data0_10 = (TH1F *)hDataCP->Clone();
  TH1F *Data10_40 = (TH1F *)hDataCP->Clone();
  TH1F *Data40_80 = (TH1F *)hDataCP->Clone();
  Data0_10->Scale(2);
  Data10_40->Scale(2);
  Data40_80->Scale(2);
  ...
  for (int cent = 0; cent < Nclasses; cent++)
  {
    ...
    GlaubV[cent] = GrDataMult->Eval(binNch[cent]);
    ...
    HData[cent] = hBvsRefMult->ProjectionY(Form("%d_hist_Impact", cent), HistData->FindBin(fMinBorder), HistData->FindBin(fMaxBorder));
  ...
  CRatio2->SaveAs(Form("%sGl_ImpactMean_%s_%s_%s.png", outDir, model0, energ0, sys));
  ...
  CutHisto(Data40_80, binNch[7], binNch[3]);
  ...
  c->SaveAs(Form("%sGlFit_%s_%s_%s.png", outDir, model0, energ0, sys));
